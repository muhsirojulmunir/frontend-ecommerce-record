...
{{--
    Struktur asli yang terbukti bekerja.
    Perubahan dari asli:
    1. Countdown dipindah ke BAWAH badge DISKON (bukan di area bawah card)
    2. Hapus min-h countdown wrapper di info area agar tidak ada gap kosong
--}}
<div class="bg-white border border-border rounded-sm hover:shadow-md transition-all duration-300 flex flex-col group relative p-2.5 sm:p-4">
    ...
    {{-- Badge SOLD / DISKON — kiri atas card (posisi asli) --}}
    <div class="absolute top-2.5 left-2.5 sm:top-4 sm:left-4 z-20 flex flex-col gap-1 items-start">
        ...
        @elseif($product->hasDiscount())
            ...
            @if($discountEndsAtTimestamp)
                <div x-data="discountCountdown({{ $discountEndsAtTimestamp }})"
                     ...
                     x-show="!expired"
                     x-cloak
                     class="inline-flex items-center gap-1 bg-white/95 border border-rose-300 text-rose-700 text-[8px] sm:text-[9px] font-bold px-1.5 py-0.5 rounded-sm shadow-sm"
                     title="Diskon berakhir {{ $discountEndsAt->translatedFormat('d M Y') }}">
                    <i class="fa-regular fa-clock text-rose-500 animate-pulse" style="font-size:7px;"></i>
                    <span class="font-mono font-black tracking-tight" x-text="display"></span>
                </div>
            @endif
        @endif
    </div>

    {{-- Tombol Keranjang — kanan atas card (posisi asli) --}}
    <div class="absolute top-2.5 right-2.5 sm:top-4 sm:right-4 z-20" x-data="{ adding: false, added: false }">
        <form
            @submit.prevent="
                if (adding) return;
                adding = true;
            ...
            <button type="submit" :disabled="adding"
                class="h-8 w-8 sm:h-9 sm:w-9 rounded-full flex items-center justify-center border shadow-md transition-all duration-200"
...
<style>
    ...
    /* Harga — selalu satu baris, tidak pernah wrap */
    .kartu-harga {
